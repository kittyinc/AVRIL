...
vrl_32bit vrl_VideoDriverMode13(vrl_VideoCommand cmd, vrl_32bit lparm1, void *pparm1)
	{
	union REGS regs;
	switch (cmd)
		{
		case VRL_VIDEO_GET_VERSION: return 1;
		case VRL_VIDEO_GET_DESCRIPTION: strncpy((char *) pparm1, "Mode 0x13 driver", lparm1); return 0;
		...
		case VRL_VIDEO_SET_PALETTE:
			{
			unsigned char *rgbtable = pparm1;
			vrl_16bit low = lparm1 >> 16, high = lparm1, i;
		...
		case VRL_VIDEO_GET_RASTER: *((vrl_Raster **) pparm1) = &our_raster; break;
		case VRL_VIDEO_BLIT:
			if ((vrl_Raster *) pparm1 != &our_raster)
				memcpy(our_raster.data, vrl_RasterGetData((vrl_Raster *) pparm1), 64000u);
			break;
